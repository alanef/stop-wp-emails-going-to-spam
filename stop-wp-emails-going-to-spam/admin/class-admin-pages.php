...
class Admin_Pages {
	...
	protected $settings_page_id = 'toplevel_page_stop-wp-emails-going-to-spam';
	...
	public function settings_setup() {
		...
		add_submenu_page(
			...
			'stop-wp-emails-going-to-spam-settings',                         /* Page Slug */
	...
	public function footer_scripts() {
		$page_hook_id = $this->settings_page_id;
		$confmsg      = esc_html__( 'Are you sure want to do this?', 'stop-wp-emails-going-to-spam' );
		?>
...
// confirm before reset
	...
	public function add_required_meta_boxes() {
		...
		if ( $this->settings_page_id == $hook_suffix ) {
			...
			add_meta_box(
				...
				__( 'Save Options', 'stop-wp-emails-going-to-spam' ),            /* Title */
            ...
            <div id="major-publishing-actions">
                <div id="delete-action">
                    <input type="submit" name="<?php echo esc_attr( "{$this->option_group}-reset" ); ?>"
                           id="<?php echo esc_attr( "{$this->option_group}-reset" ); ?>" class="button"
                           value="Reset Settings">
                </div><!-- #delete-action -->
                <div id="publishing-action">
                    <span class="spinner"></span>
					<?php submit_button( esc_attr__( 'Save', 'stop-wp-emails-going-to-spam' ), 'primary', 'submit', false ); ?>
                </div>
                <div class="clear"></div>
	...
	public function reset_sanitize( $settings ) {
		/* Add Update Notice */
		if ( ! empty( $settings ) ) {
			add_settings_error( $this->option_group, '', esc_html__( 'Settings reset to defaults.', 'stop-wp-emails-going-to-spam' ), 'updated' );
			/* Delete Option */
			$this->delete_options();
		}
